...
def current_users(events):
  ...
  for event in events:
    ...
    if event.type == "login" and machines[event.machine] == None:
      machines[event.machine] = event.user
    elif event.type == "logout" and machines[event.machine] == None:
      machines[event.machine] = None
    return machines
...
# No output should be generated from running the custom function definitions above. To check that our code is doing everything it's supposed to do, we need an Event class.
...
# Uh oh. The code in the previous cell produces an error message. This is because we have a user in our events list that was logged out of a machine he was not logged into. Do you see which user this is? 
...
# Remember when you have finished making your edits, rerun that cell as well as the cell that feeds the events list into our custom_users function to see whether the error message has been fixed. 
# Once the error message has been cleared and you have correctly outputted a dictionary with machine names as keys, your custom functions are properly finished. Great!
